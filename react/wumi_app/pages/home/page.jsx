...
export default class extends Component {
    ...
    componentWillMount() {
        ...
        //         console.log('geoCurrentPosition', position);
    ...
    loadPoiData ({ latitude, longitude }) {
        // 请求代理 代替组件向某接口发送请求
        fetch(`/restapi/bgs/poi/reverse_geo_coding?latitude=${latitude}&longitude=${longitude}`).then(res => {
            if(res.status === 200) {
                res.json().then(data => {
                    this.setState({
                        address: data.name
        ...
        return (
            <div className={styles.home}>
                ...
                <StickyContainer>
                    ...
                    <Grid className={styles.grid} data={this.getTypeData()} columnNum={5} carouselMaxRow={2}
                        hasLine={false} activeStyle={false} isCarousel renderItem={(item) => (
                        ...
                        <ListView dataSource={this.dataSource} renderFooter={() => (<div style={{padding: 30,
                        textAlign: 'center'}}>{this.state.isLoading ? '加载中...': '暂无更多'}</div>)} renderRow={this.renderRow} pageSize={4} useBodyScroll scrollRenderAheadDistance={500}
                        onEndReachedThreshold={10} onEndReached={this.onEndReached} />
                </StickyContainer>
            </div>
        )
    }
